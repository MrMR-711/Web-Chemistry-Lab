...
async function sendMessage() {
  ...
  try {
    ...
    hideTypingIndicator();
    addMessageToUI('ai', `متاسفانه خطایی رخ داد: ${error.message}`, null, true);
  } finally {
    saveChatsToStorage();
    renderHistoryList();
...
function startNewChat() {
  ...
  chats[newId] = {
    ...
    messages: [{
      role: 'model',
      parts: [{ text: 'سلام! من Gemini هستم، هوش مصنوعی گوگل. چطور می‌تونم بهت کمک کنم؟ **راهنما: اگر در ایران زندگی می‌کنید لطفا از تحریم شکن یا فیلتر شکن استفاده کنید.**' }]
    }]
  };
  loadChat(newId);
  saveChatsToStorage();
...
function addMessageToUI(sender, content, imageUrl = null, isError = false) {
  ...
  const header = sender === 'user'
    ? `<div class="message-header"><i class="fas fa-user"></i>شما</div>`
...
function handleImageAttachment(event) {
  ...
  reader.onloadend = () => {
    ...
    messageInput.placeholder = `تصویر ضمیمه شد. پیام خود را بنویسید...`;
    handleInputChange();
  };
...
function clearAllHistory() {
  if (confirm('آیا از پاک کردن تمام تاریخچه گفتگوها اطمینان دارید؟ این عمل غیرقابل بازگشت است.')) {
    chats = {};
    currentChatId = null;
